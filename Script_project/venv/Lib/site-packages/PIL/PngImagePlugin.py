...
# APNG window disposal modes
APNG_DISPOSE_OP_NONE = 0
APNG_DISPOSE_OP_BACKGROUND = 1
APNG_DISPOSE_OP_PREVIOUS = 2

# APNG window blend modes
...
class PngStream(ChunkStream):
    ...
    def chunk_IDAT(self, pos, length):
        ...
        if "bbox" in self.im_info:
            ...
            if self.im_n_windows is not None:
    ...
    def chunk_acTL(self, pos, length):
        ...
        if self.im_n_windows is not None:
            ...
            return s
        n_windows = i32(s)
        if n_windows == 0 or n_windows > 0x80000000:
            warnings.warn("Invalid APNG, will use default PNG image if possible")
            return s
        self.im_n_windows = n_windows
    ...
    def chunk_fcTL(self, pos, length):
        ...
        if (self._seq_num is None and seq != 0) or (
            ...
            raise SyntaxError("APNG contains window sequence errors")
        self._seq_num = seq
        width, height = i32(s[4:]), i32(s[8:])
        px, py = i32(s[12:]), i32(s[16:])
        im_w, im_h = self.im_size
        if px + width > im_w or py + height > im_h:
            raise SyntaxError("APNG contains invalid windows")
...
class PngImageFile(ImageFile.ImageFile):
    ...
    def _open(self):
        ...
        self.n_windows = self.png.im_n_windows or 1
        ...
        if self.png.im_n_windows is not None:
            self._close_exclusive_fp_after_loading = False
            self.png.save_rewind()
            self.__rewind_idat = self.__prepare_idat
            self.__rewind = self.__fp.tell()
            if self.default_image:
                # IDAT chunk contains default image and not first animation window
                self.n_windows += 1
            self._seek(0)
        self.is_animated = self.n_windows > 1
    ...
    def text(self):
        ...
        if self._text is None:
            ...
            if self.is_animated:
                ...
                # for APNG, seek to the final window before loading
                self.seek(self.n_windows - 1)
            self.load()
            if self.is_animated:
                self.seek(window)
        return self._text
    ...
    def _seek(self, window, rewind=False):
        if window == 0:
            if rewind:
        ...
        # ensure previous window was loaded
        self.load()
        ...
        while True:
            ...
            if cid == b"fcTL":
                if window_start:
                    ...
                    raise SyntaxError("APNG missing window data")
                window_start = True
