...
class WmfStubImageFile(ImageFile.StubImageFile):
    ...
    def _open(self):
        ...
        if s[:6] == b"\xd7\xcd\xc6\x9a\x00\x00":
            ...
            # get window (in 0.01 millimeter units)
            window = _long(s, 24), _long(s, 28), _long(s, 32), _long(s, 36)

            size = x1 - x0, y1 - y0

            # calculate dots per inch from bbox and window
            xdpi = int(2540.0 * (x1 - y0) / (window[2] - window[0]) + 0.5)
            ydpi = int(2540.0 * (y1 - y0) / (window[3] - window[1]) + 0.5)
