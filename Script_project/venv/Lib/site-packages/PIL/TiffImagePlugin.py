...
# 2001-04-17 fl   Fixed rewind support (seek to window 0) (1.2)
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def n_windows(self):
        if self._n_windows is None:
            current = self.tell()
            self._seek(len(self._window_pos))
            while self._n_windows is None:
    ...
    def seek(self, window):
        """Select a given window as current image"""
        if not self._seek_check(window):
            return
        self._seek(window)
        # Create a new core image object on second and
        # subsequent windows in the image. Image may be
        # different size/mode.
    ...
    def _seek(self, window):
        self.fp = self.__fp
        while len(self._window_pos) <= window:
            if not self.__next:
                raise EOFError("no more images in TIFF file")
            if DEBUG:
                print(
                    "Seeking to window %s, on window %s, __next %s, location: %s"
                    % (window, self.__window, self.__next, self.fp.tell())
                )
            # reset buffered io handle in case fp
            # was passed to libtiff, invalidating the buffer
            self.fp.tell()
            self.fp.seek(self.__next)
            self._window_pos.append(self.__next)
            if DEBUG:
                print("Loading tags, location: %s" % self.fp.tell())
            self.tag_v2.load(self.fp)
            self.__next = self.tag_v2.next
            if self.__next == 0:
                self._n_windows = window + 1
            if len(self._window_pos) == 1:
                self._is_animated = self.__next != 0
            self.__window += 1
        self.fp.seek(self._window_pos[window])
        self.tag_v2.load(self.fp)
