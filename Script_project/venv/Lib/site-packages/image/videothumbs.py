...
def generate_thumb(storage, video_path, thumb_size=None, format='jpg', windows=100):
    histogram = []
    ...
    windowmask = "%s%s%s%s" % (TMP_DIR,
                              name.split('/')[-1].split('.')[0] + str(time.time()),
                              '.%d.',
                              format)
    # ffmpeg command for grabbing N number of windows
    cmd = "/usr/bin/ffmpeg -y -t 00:00:05 -i '%s' '%s'" % (path, windowmask)
    ...
    # We don't know how many windows we capture. We just captured the first 5 seconds, so keep removing until not found
